...
class SARNN(nn.Module):
    ...
    def __init__(
        ...
        rec_in = state_dim + self.key_dim * 2
        self.rec = nn.LSTMCell(rec_in, union_dim)  # LSTM cell

        # Joint Decoder
        self.decoder_joint = nn.Sequential(
            nn.Linear(union_dim, state_dim), activation
        )  # Linear layer and activation

        # Point Decoder
        self.decoder_point = nn.Sequential(
            nn.Linear(union_dim, self.key_dim * 2), activation
        )  # Linear layer and activation
