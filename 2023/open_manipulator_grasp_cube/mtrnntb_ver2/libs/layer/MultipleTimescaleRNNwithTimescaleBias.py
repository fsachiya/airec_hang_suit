...
class MTRNNTBCell(nn.Module):
    ...
    def __init__(
        ...
        super(MTRNNTBCell, self).__init__()

        self.input_dim = input_dim
        self.fast_dim = fast_dim
        self.slow_dim = slow_dim
        self.init_fast_tau = (fast_tau_range["max"]+fast_tau_range["min"])/2.0
        self.fast_tau_tb = nn.Parameter(torch.normal(mean=0, std=1, size=(1, 100)), requires_grad=True)
        self.fast_tau_range = fast_tau_range
        self.slow_tau = slow_tau
        self.use_bias = use_bias
        self.use_pb = use_pb
        self.g = (self.fast_tau_range["max"]-self.fast_tau_range["min"])/2.0
        ...
        self.fast_tau_tb_layer = nn.Sequential(
            nn.Linear(100,10),
            self.activation,
            nn.Linear(10,1),
            nn.Tanh()
        )
        # self.fast_tau_tb_layer = nn.Linear(100,1)
    ...
    def forward(self, x, state=None, pb=None):
        ...
        _fast_tau_tb = self.g * self.fast_tau_tb_layer(self.fast_tau_tb)
