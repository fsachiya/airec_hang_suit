...
for loop_ct in range(nloop):
    ...
    # predict rnn
    y_image, y_joint, state, fast_tau = model(x_img, x_joint, state)
    ...
    expand_fast_tau_list.append(tensor2numpy(fast_tau[0,0]))
    ...
    print("loop_ct:{}, joint:{}".format(loop_ct, expand_pred_joint))
...
fast_tau_list = np.array(expand_fast_tau_list)[10:]
...
ani.save(f'./output/test_pos_{idx}/{params["tag"]}/MTRNNTB_{params["tag"]}.gif')
...
# fig = plt.figure()
# plt.plot(range(len(fast_tau_list)), fast_tau_list, linestyle='solid', c='k', label="online")
# plt.plot(range(len(fast_tau_list)), fast_tau_list.mean()*np.ones_like(fast_tau_list), linestyle='dashed', c='r', label="average")
# plt.grid()
# # plt.ylim(0, 0.05)
# plt.xlabel("step")
# plt.ylabel("fast_tau")
# plt.legend(bbox_to_anchor=(1, 1), loc='upper right', borderaxespad=0) # fontsize=18
# plt.savefig(f'./output/test_pos_{idx}/fast_tau_trend.png')
# plt.clf()
# plt.close()

result = {
    "context_size": {"cf": params["fast_context"], "cs": params["slow_context"]},
    "fast_tau": float(fast_tau_list[-1]),
