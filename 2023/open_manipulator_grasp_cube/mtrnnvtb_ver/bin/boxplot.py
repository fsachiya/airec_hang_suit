...
for idx in [1,3]:
        # tb_ver
    test_loss_idx = []
    result_json_path = f'/home/fujita/job/2023/open_manipulator_grasp_cube/mtrnntb_ver/output/test_pos_{idx}/*/result.json'
    result_json_files = glob.glob(result_json_path)
    for result_json_file in result_json_files:
        with open(result_json_file) as f:
            result = json.load(f)
...
for idx in [1,3]:
    ...
    for result_json_file in result_json_files:
        with open(result_json_file) as f:
            result = json.load(f)
            test_loss_idx.append(result["loss"]["test"])
    vtb_test_losses.append(test_loss_idx)

# import ipdb; ipdb.set_trace()
normal_test_losses = np.array(normal_test_losses)
tb_test_losses = np.array(tb_test_losses).reshape(2,1,-1)
vtb_test_losses = np.array(vtb_test_losses).reshape(2,1,-1)
test_losses = np.concatenate([normal_test_losses, tb_test_losses, vtb_test_losses], 1)
...
for i, idx in enumerate([1,3]):
    ...
    x_ticks = [0, 1, 2, 3, 4]  # 目盛りの位置を適切に設定
    ax.set_xticks(x_ticks)
    ax.set_xticklabels(["tau_2", "tau_4", "tau_6", "tb_tau", "vtb_tau"])
