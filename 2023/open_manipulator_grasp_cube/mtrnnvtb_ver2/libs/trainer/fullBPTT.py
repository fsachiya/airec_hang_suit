        ...
        for n_batch, ((x_img, x_joint), (y_img, y_joint)) in enumerate(data):
            ...
            for t in range(T - 1):
                _yi_hat, _yv_hat, state, fast_tau = self.model(
                    x_img[:, t], x_joint[:, t], prev_fast_tau, state
                )
                ...
                # # import ipdb; ipdb.set_trace()
                # img_loss = nn.MSELoss()(yi_hat[:,0], y_img[:, t+1, :]) * self.loss_weights[0]
                # joint_loss = nn.MSELoss()(yv_hat[:,0], y_joint[:, t+1, :]) * self.loss_weights[1]
