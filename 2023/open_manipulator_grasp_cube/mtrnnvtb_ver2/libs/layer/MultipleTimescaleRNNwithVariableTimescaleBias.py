...
class MTRNNVTBCell(nn.Module):
    ...
    def __init__(
        ...
        super(MTRNNVTBCell, self).__init__()

        self.input_dim = input_dim
        self.fast_dim = fast_dim
        self.slow_dim = slow_dim
        self.init_fast_tau = (fast_tau_range["max"]+fast_tau_range["min"])/2.0
        ...
        self.fast_tau_vtb_layer = nn.Sequential(
            nn.Linear(fast_dim,10),
            nn.BatchNorm1d(10),
            self.activation,
            nn.Linear(10,1),
            nn.BatchNorm1d(1),
    ...
    def forward(self, x, prev_fast_tau=None, state=None, pb=None):
        ...
        # g = (self.fast_tau_range["max"]-self.fast_tau_range["min"])/2.0
        # _fast_tau_tb = g * nn.Tanh()(self.fast_tau_tb_layer(self.fast_tau_tb))
        # fast_tau = self.init_fast_tau + _fast_tau_tb
        # g = (self.fast_tau_range["max"]-self.fast_tau_range["min"])/2.0

        g = (self.fast_tau_range["max"]-self.fast_tau_range["min"])/2.0
        _fast_tau_vtb = g * self.fast_tau_vtb_layer(prev_u_fast)
        fast_tau = self.init_fast_tau + _fast_tau_vtb
        
        if prev_fast_tau is not None:
            pass
        else:
            prev_fast_tau = fast_tau
        
        fast_tau = (fast_tau + prev_fast_tau)/2.0
        ...
        return new_h_fast, new_h_slow, new_u_fast, new_u_slow, fast_tau
