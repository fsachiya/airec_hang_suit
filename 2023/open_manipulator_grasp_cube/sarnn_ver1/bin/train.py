...
# model.load_state_dict(torch.load('/home/shigeki/Documents/sachiya/work/eipl/eipl/works/open_manipulator_grasp_cube/sarnn_ver/log/20230901_1501_29/SARNN.pth'))
...
with tqdm(range(args.epoch)) as pbar_epoch:
    for epoch in pbar_epoch:
        # train and test
        train_loss, train_img_loss, train_joint_loss, train_pt_loss = trainer.process_epoch(train_loader)
        test_loss, test_img_loss, test_joint_loss, test_pt_loss = trainer.process_epoch(test_loader, training=False)
        writer.add_scalar("Loss/train_loss", train_loss, epoch)
        writer.add_scalar("Loss/train_img_loss", train_img_loss, epoch)
        writer.add_scalar("Loss/train_joint_loss", train_joint_loss, epoch)
        writer.add_scalar("Loss/train_pt_loss", train_pt_loss, epoch)

        writer.add_scalar("Loss/test_loss", test_loss, epoch)
        writer.add_scalar("Loss/test_img_loss", test_img_loss, epoch)
        writer.add_scalar("Loss/test_joint_loss", test_joint_loss, epoch)
        writer.add_scalar("Loss/test_pt_loss", test_pt_loss, epoch)

        # early stop
        save_ckpt, _ = early_stop(test_loss)

        if save_ckpt:
            trainer.save(epoch, [   train_loss, \
                                    # train_img_loss, \
                                    # train_joint_loss, \
                                    # train_pt_loss, \
                                    test_loss
