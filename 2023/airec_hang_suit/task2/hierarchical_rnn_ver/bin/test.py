...
# Inference
loss_weights = [params["key_point_loss"], 
                params["joint_loss"], 
                params["cmd_loss"]]
...
key_point_loss = (enc_pts_data - dec_pts_data)**2 * loss_weights[0]
joint_loss = (yv_hat_data - yv_data[:, 1:])**2 * loss_weights[1]
cmd_loss = (yc_hat_data - yc_data[:, 1:])**2 * loss_weights[2]
key_point_loss = key_point_loss.mean(dim=-1)
joint_loss = joint_loss.mean(dim=-1)
cmd_loss = cmd_loss.mean(dim=-1)
# key_point_loss = nn.MSELoss()(enc_pts_data, dec_pts_data) * loss_weights[0]
# joint_loss = nn.MSELoss()(yv_hat_data, yv_data[:, 1:]) * loss_weights[1]
# cmd_loss = nn.MSELoss()(yc_hat_data, yc_data[:, 1:]) * loss_weights[2]
loss = key_point_loss + joint_loss + cmd_loss
...
# img
# _img_data = tensor2numpy(img_data.permute(0,1,3,4,2))
...
def anim_update(i):
    ...
    for j in range(params["key_point_num"]):
        ax[0][0].plot(_enc_pts_data[i, j, 0], _enc_pts_data[i, j, 1], "bo", markersize=6)  # encoder
        ax[0][0].plot(
            _dec_pts_data[i, j, 0], _dec_pts_data[i, j, 1], "rx", markersize=6, markeredgewidth=2
        )  # decoder
    ax[0][0].axis("off")
    ax[0][0].set_title("cam img")
    
    # plot pred img
    ax[1][0].imshow(_pred_img_data[i, :, :, ::-1])
    ax[1][0].axis("off")
    ...
    # plot left command
    ax[0][2].set_ylim(-1.0, 2.0)
    ax[0][2].set_xlim(0, T)
    ax[0][2].plot(_hand_cmd_data[1:, :3], linestyle="dashed", c="k")
    # command has 3
    for cmd_idx in range(3):
        ax[0][2].plot(np.arange(i + 1), _pred_hand_cmd_data[: i + 1, cmd_idx])
    # ax[0][2].set_xlabel("Step")
    ax[0][2].set_title("right hand command")
