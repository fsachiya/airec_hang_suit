...
for t in range(T-1):
    # predict rnn
    yi_hat, yv_hat, yp_hat, enc_pts, dec_pts, states, xi_feat = model(
        x_img[:, t], x_vec[:, t], x_press[:, t], states=states  # step=500, 
    )
...
roll_pt_loss = (roll_enc_pts_data - roll_dec_pts_data)**2 * loss_w_dic["k"]

delta_enc_pt_loss = (enc_pts_data - roll_enc_pts_data)**2 * loss_w_dic["k"]
delta_dec_pt_loss = (dec_pts_data - roll_dec_pts_data)**2 * loss_w_dic["k"]

img_loss = img_loss.mean(dim=(2,3,4))
vec_loss = vec_loss.mean(dim=-1)
press_loss = press_loss.mean(dim=-1)
pt_loss = pt_loss.mean(dim=-1)
roll_pt_loss = roll_pt_loss.mean(dim=-1)
delta_enc_pt_loss = delta_enc_pt_loss.mean(dim=-1)
delta_dec_pt_loss = delta_dec_pt_loss.mean(dim=-1)
