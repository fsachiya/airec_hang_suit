...
class fullBPTTtrainer:
    ...
    def process_epoch(self, data, training=True):
        ...
        total_loss = 0.0
        for n_batch, ((x_left_img, x_right_img, x_vec), 
                      (y_left_img, y_right_img, y_vec)) in enumerate(data):   # , x_press, y_press
            ...
            for t in range(T - 1):
                [yli_hat, yri_hat, yv_hat, 
                 left_enc_pts, left_dec_pts, right_enc_pts, right_dec_pts, states] = self.model(    # , yp_hat
                    x_left_img[:, t], x_right_img[:, t], x_vec[:, t], states    # , x_press[:, t]
                )
            ...
            left_img_loss = criterion(yli_hat_data, y_left_img[:, 1:]) * self.loss_w_dic["i"]
            right_img_loss = criterion(yri_hat_data, y_right_img[:, 1:]) * self.loss_w_dic["i"]
            ...
            left_pt_loss = criterion(left_enc_pts_data, left_dec_pts_data)  * self.loss_w_dic["k"] # * self.scheduler(self.loss_weights[2])
            right_pt_loss = criterion(right_enc_pts_data, right_dec_pts_data) * self.loss_w_dic["k"] # * self.scheduler(self.loss_weights[2])
            pt_loss = left_pt_loss + right_pt_loss
            # pt_loss = left_pt_loss * right_pt_loss
            
            stereo_enc_pts_x_loss = criterion(_left_enc_pts_data[:,:,:,0], _right_enc_pts_data[:,:,:,0]) * (self.loss_w_dic["k"] * 0.1)
            stereo_dec_pts_x_loss = criterion(_left_dec_pts_data[:,:,:,0], _right_dec_pts_data[:,:,:,0]) * (self.loss_w_dic["k"] * 0.1)
            stereo_enc_pts_y_loss = criterion(_left_enc_pts_data[:,:,:,1], _right_enc_pts_data[:,:,:,1]) * (self.loss_w_dic["k"])
            stereo_dec_pts_y_loss = criterion(_left_dec_pts_data[:,:,:,1], _right_dec_pts_data[:,:,:,1]) * (self.loss_w_dic["k"])
            ...
            left_enc_var_loss = 1/torch.var(_left_enc_pts_data) * 0.0001
            left_dec_var_loss = 1/torch.var(_left_dec_pts_data) * 0.0001
            right_enc_var_loss = 1/torch.var(_right_enc_pts_data) * 0.0001
            right_dec_var_loss = 1/torch.var(_right_dec_pts_data) * 0.0001
            var_loss = left_enc_var_loss + left_dec_var_loss + right_enc_var_loss + right_dec_var_loss

            
            # huber = nn.HuberLoss(delta=0.1)
            # stereo_enc_pts_x_loss = huber(_left_enc_pts_data[:,:,:,0], _right_enc_pts_data[:,:,:,0]) * (self.loss_w_dic["k"] * 0.2)
            # stereo_dec_pts_x_loss = huber(_left_dec_pts_data[:,:,:,0], _right_dec_pts_data[:,:,:,0]) * (self.loss_w_dic["k"] * 0.2)
            # stereo_enc_pts_y_loss = huber(_left_enc_pts_data[:,:,:,1], _right_enc_pts_data[:,:,:,1]) * (self.loss_w_dic["k"] * 2)
            # stereo_dec_pts_y_loss = huber(_left_dec_pts_data[:,:,:,1], _right_dec_pts_data[:,:,:,1]) * (self.loss_w_dic["k"] * 2)
            # stereo_xy_loss = stereo_enc_pts_x_loss + stereo_dec_pts_x_loss + stereo_enc_pts_y_loss + stereo_dec_pts_y_loss
            # # stereo_y_loss = stereo_enc_pts_y_loss + stereo_dec_pts_y_loss
            # # stereo_y_loss = stereo_enc_pts_y_loss * stereo_dec_pts_y_loss
            ...
            #         stereo_enc_pt_x_loss = criterion(_left_enc_pts_data[:,:,i,0], _right_enc_pts_data[:,:,i,0])
            #         stereo_dec_pt_x_loss = criterion(_left_dec_pts_data[:,:,i,0], _right_dec_pts_data[:,:,i,0])
            #         stereo_enc_pt_y_loss = criterion(_left_enc_pts_data[:,:,i,1], _right_enc_pts_data[:,:,i,1])
            #         stereo_dec_pt_y_loss = criterion(_left_dec_pts_data[:,:,i,1], _right_dec_pts_data[:,:,i,1])
            ...
            # for i in range(_left_enc_pts_data.shape[2]):
            #     if (_left_enc_pts_data[:,:,i,0].mean() > self.approx_parallax_ratio) and (_right_enc_pts_data[:,:,i,0].mean() < (1-self.approx_parallax_ratio)):
            #         stereo_enc_pt_x_loss = huber(_left_enc_pts_data[:,:,i,0], _right_enc_pts_data[:,:,i,0])
            #         stereo_dec_pt_x_loss = huber(_left_dec_pts_data[:,:,i,0], _right_dec_pts_data[:,:,i,0])
            #         stereo_enc_pt_y_loss = huber(_left_enc_pts_data[:,:,i,1], _right_enc_pts_data[:,:,i,1])
            #         stereo_dec_pt_y_loss = huber(_left_dec_pts_data[:,:,i,1], _right_dec_pts_data[:,:,i,1])
            #         stereo_enc_pt_x_loss_list.append(stereo_enc_pt_x_loss)
            #         stereo_dec_pt_x_loss_list.append(stereo_dec_pt_x_loss)
            #         stereo_enc_pt_y_loss_list.append(stereo_enc_pt_y_loss)
            #         stereo_dec_pt_y_loss_list.append(stereo_dec_pt_y_loss)
            # stereo_enc_pts_x_loss = torch.stack(stereo_enc_pt_x_loss_list).mean() * (self.loss_w_dic["k"] * 0.2)
            # stereo_dec_pts_x_loss = torch.stack(stereo_dec_pt_x_loss_list).mean() * (self.loss_w_dic["k"] * 0.2)
            # stereo_enc_pts_y_loss = torch.stack(stereo_enc_pt_y_loss_list).mean() * (self.loss_w_dic["k"] * 2)
            # stereo_dec_pts_y_loss = torch.stack(stereo_dec_pt_y_loss_list).mean() * (self.loss_w_dic["k"] * 2)
            # stereo_xy_loss = stereo_enc_pts_x_loss + stereo_dec_pts_x_loss + stereo_enc_pts_y_loss + stereo_dec_pts_y_loss
            # # stereo_y_loss = stereo_enc_pts_y_loss + stereo_dec_pts_y_loss
       
       
            loss = left_img_loss + right_img_loss + pt_loss + vec_loss  # + press_loss    + stereo_xy_loss   + var_loss
            # loss = left_img_loss * right_img_loss * pt_loss * vec_loss * stereo_y_loss # + press_loss    + stereo_xy_loss
