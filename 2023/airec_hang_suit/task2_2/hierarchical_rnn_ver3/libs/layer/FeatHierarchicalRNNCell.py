...
class FeatHierarchicalRNNCell(nn.Module):
    # :: HierachicalRNNCell
    ...
    def __init__(
        ...
        super(FeatHierarchicalRNNCell, self).__init__()
        ...
        # num_heads = 5
        # self.urnn_attn = nn.MultiheadAttention(embed_dim=srnn_hid_dim*self.modal_num, num_heads=num_heads)
    ...
    def get_initial_states(self, x):
        ...
        states = [prev_fsrnn_state, prev_ksrnn_state, prev_vsrnn_state, prev_psrnn_state, prev_urnn_state]
        return states
    ...
    def forward(
        ...
        if states is not None:
            prev_fsrnn_state, prev_ksrnn_state, prev_vsrnn_state, prev_psrnn_state, prev_urnn_state = states
        else:
            prev_fsrnn_state, prev_ksrnn_state, prev_vsrnn_state, prev_psrnn_state, prev_urnn_state = self.get_initial_states(xk)
        ...
        urnn_input = torch.cat((prev_fsrnn_state[0],
                                prev_ksrnn_state[0], 
                                prev_vsrnn_state[0],
                                prev_psrnn_state[0]), axis=-1)
        
        # attn_xu, attn_wu = self.urnn_attn(urnn_input, urnn_input, urnn_input)
        
        new_urnn_state = self.URNN(urnn_input, prev_urnn_state)   # urnn_input
        urnn_out = self.urnn_out_layer(new_urnn_state[0])
        prev_fsrnn_hid, prev_ksrnn_hid, prev_vsrnn_hid, prev_psrnn_hid = torch.split(
        ...
        states = [new_fsrnn_state, new_ksrnn_state, new_vsrnn_state, new_psrnn_state, new_urnn_state]
