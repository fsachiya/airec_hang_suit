...
try:
    from libs.layer import VariableHierarchicalRNNCell
except:
    from layer import VariableHierarchicalRNNCell

class VHSARNN(nn.Module):
    def __init__(
        self,
        srnn_hid_dim=50,
        urnn_hid_dim=20,
        ...
        # )
        self.img_encoder = nn.Sequential(
            nn.Conv2d(3, 8, 3, 1, 1),  # 3, 64, 64 -> 16, 64, 64
            activation,
            nn.Conv2d(8, 8, 3, 1, 1),  # 16, 64, 64 -> 32, 64, 64
            activation,
            nn.Conv2d(8, 16, 3, 1, 1),  # 16, 64, 64 -> 32, 64, 64
            activation,
            nn.Conv2d(16, 16, 3, 1, 1),  # 16, 64, 64 -> 32, 64, 64
            activation,
            nn.Conv2d(16, 32, 3, 1, 1),  # 16, 64, 64 -> 32, 64, 64
            activation,
            nn.Conv2d(32, 32, 3, 1, 1),  # 16, 64, 64 -> 32, 64, 64
            activation,
            nn.Conv2d(32, self.k_dim, 3, 1, 1),  # 32, 64, 64 -> 8, 64, 64
            activation,
        )
        ...
        self.vhrnn = VariableHierarchicalRNNCell(
            srnn_input_dims={"k": key_dim, "v": vec_dim, "p": press_dim}, # "f": feat_dim, 
        ...
        self.img_decoder = nn.Sequential(
            nn.ConvTranspose2d(self.k_dim, 32, 3, 1, 1),  # Transposed Convolutional layer 1
            activation,
            nn.ConvTranspose2d(32, 32, 3, 1, 1),  # Transposed Convolutional layer 1
            activation,
            nn.ConvTranspose2d(32, 16, 3, 1, 1),  # Transposed Convolutional layer 2
            activation,
            nn.ConvTranspose2d(16, 16, 3, 1, 1),  # Transposed Convolutional layer 2
            activation,
            nn.ConvTranspose2d(16, 8, 3, 1, 1),  # Transposed Convolutional layer 3
            activation,
            nn.ConvTranspose2d(8, 8, 3, 1, 1),  # Transposed Convolutional layer 3
            activation,
            nn.ConvTranspose2d(8, 3, 3, 1, 1),  # Transposed Convolutional layer 3
            activation,
        )
        # self.img_decoder = nn.Sequential(
        #     nn.ConvTranspose2d(self.k_dim, 32, 3, 1, 1),  # Transposed Convolutional layer 1
    ...
    def forward(self, xi, xv, xp, step, states=None):
        # Encode input image
        ...
        states = self.vhrnn(xk, xv, xp, step, states) # xf, 

        # yi_feat = self.feat_decoder(states[0][0])
        dec_pts = self.point_decoder(states[0][0])  # Decode points
        yv = self.vec_decoder(states[1][0])  # Decode joint prediction
        yp = self.press_decoder(states[2][0])  # Decode joint prediction
        
        # Reshape decoded points
        dec_pts_in = dec_pts.reshape(-1, self.k_dim, 2)
        heatmap = self.issm(dec_pts_in)  # Inverse Spatial Softmax
        # hid
        yi_feat = torch.mul(heatmap, xi_feat)  # Multiply heatmap with image feature `im_hid`        
