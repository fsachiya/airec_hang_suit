...
import torch
from torch.optim.lr_scheduler import ReduceLROnPlateau, CosineAnnealingWarmRestarts, LinearLR, MultiStepLR
import argparse
...
_img_data = resize_img(raw_img_data, (args.img_size, args.img_size))
import ipdb; ipdb.set_trace()

# _diff_img_data = np.array([np.abs(_img_data[:,i+1] - _img_data[:,i]) for i in range(_img_data.shape[1]-1)])
# _diff_img_data = _diff_img_data.transpose((1,0,2,3,4))
# _diff_img_data = np.concatenate((np.expand_dims(np.zeros_like(_diff_img_data[:,0]), axis=1),
#                                  _diff_img_data),
#                                 axis=1)
# # _diff_img_data = _diff_img_data.mean(axis=4)
# # 動画ファイルの設定
# diff_img_data = _diff_img_data[0]
# output_video = './fig/diff_img.mp4'
# fps = 10
# frame_width = diff_img_data.shape[2]
# frame_height = diff_img_data.shape[1]
# fourcc = cv2.VideoWriter_fourcc(*'mp4v')  # MP4形式を指定
# video_writer = cv2.VideoWriter(output_video, fourcc, fps, (frame_width, frame_height))
# # フレームごとに動画に書き込む
# for frame in diff_img_data:
#     # _frame = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
#     # kernel_size = 3
#     # _frame = cv2.blur(frame, (kernel_size, kernel_size))
#     # _, threshold = cv2.threshold(frame_diff_gray, 30, 255, cv2.THRESH_BINARY)
#     import ipdb; ipdb.set_trace()
#     _frame = np.where(frame > 10, 255, 0)
#     # import ipdb; ipdb.set_trace()
...
trainer = fullBPTTtrainer(model, 
                          ...
                          device=device)
# trainer = featfullBPTTtrainer(model, 
#                           optimizer, 
#                           loss_w_dic=loss_w_dic, 
#                           device=device)
...
#                      end_factor=0.1*args.lr, 
#                      total_iters=40)
scheduler = MultiStepLR(optimizer, 
                        milestones=[40,200], 
                        gamma=0.1)

if not (args.state_dir == None):
...
with tqdm(range(args.epoch)) as pbar_epoch:
    for epoch in pbar_epoch:
        # train and test
        train_loss = trainer.process_epoch(train_loader, epoch) # with step
        with torch.no_grad():
            test_loss = trainer.process_epoch(test_loader, epoch, training=False) # with step
        ...
        if (epoch % 100 == 0) or (epoch % 10 == 0 and epoch < 200) or (epoch == args.epoch-1):
            save_name = os.path.join(log_dir_path, f"HierarchicalRNN_{epoch}.pth")
            trainer.save(epoch, [train_loss, test_loss], save_name)
        ...
        plt.figure()
        plt.plot(range(len(lr_list)), lr_list, label="train")
        plt.grid()
        plt.legend()
        plt.savefig(f'{log_dir_path}/lr_trend.png')
        plt.close()
...
plt.plot(range(len(test_loss_list[10:])), test_loss_list[10:], label="test")
plt.grid()
plt.legend()
plt.savefig(f'{log_dir_path}/loss_trend.png')
plt.close()
