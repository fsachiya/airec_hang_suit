...
try:
    from libs.layer import StereoHierarchicalRNNCell
except:
    from layer import StereoHierarchicalRNNCell


class StereoHSARNN(nn.Module):
    def __init__(
        self,
        # rnn_dim,
        # union_dim,
        ...
        super(StereoHSARNN, self).__init__()
        ...
        # self.rec
        self.hrnn = StereoHierarchicalRNNCell(srnn_input_dims={"k": key_dim, "v": vec_dim, "p": press_dim}, 
    ...
    def forward(self, xli, xri, xv, xp, states=None):
        # Encode input image
        # im_hid
        xli_feat = self.img_encoder(xli)
        left_enc_pts, _ = self.pos_encoder(xli)
        xri_feat = self.img_encoder(xri)
        right_enc_pts, _ = self.pos_encoder(xri)
        ...
        left_enc_pts = left_enc_pts.reshape(-1, self.k_dim * 2)
        xlk = left_enc_pts
        right_enc_pts = right_enc_pts.reshape(-1, self.k_dim * 2)
        xrk = right_enc_pts
        # hid = torch.cat([enc_pts, xv], -1)
        # rnn_hid = self.rec(xv, enc_pts, state)  # LSTM forward pass
        
        # [new_left_ksrnn_state, new_right_ksrnn_state, new_vsrnn_state, new_psrnn_state, new_urnn_state]
        states = self.hrnn(xlk, xrk, xv, xp, states) # enc_pts
        left_dec_pts = self.decoder_point(states[0][0])  # Decode points
        right_dec_pts = self.decoder_point(states[1][0])  # Decode points
        yv = self.decoder_joint(states[2][0])  # Decode joint prediction
        yp = self.decoder_press(states[3][0])  # Decode joint prediction
        
        # Reshape decoded points
        left_dec_pts_in = left_dec_pts.reshape(-1, self.k_dim, 2)
        left_heatmap = self.issm(left_dec_pts_in)  # Inverse Spatial Softmax
        right_dec_pts_in = right_dec_pts.reshape(-1, self.k_dim, 2)
        right_heatmap = self.issm(right_dec_pts_in)  # Inverse Spatial Softmax
        # hid
        xli_feat = torch.mul(left_heatmap, xli_feat)  # Multiply heatmap with image feature `im_hid`
        xri_feat = torch.mul(right_heatmap, xri_feat)  # Multiply heatmap with image feature `im_hid`

        yli = self.decoder_image(xli_feat)  # Decode image
        yri = self.decoder_image(xri_feat)  # Decode image
        return [yli, yri, yv, yp, 
                left_enc_pts, left_dec_pts, right_enc_pts, right_dec_pts, states]# rnn_hid
