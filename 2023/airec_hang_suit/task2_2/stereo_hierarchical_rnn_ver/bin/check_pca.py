...
raw_left_img_data = raw_left_img_data[[4,6,9,18,12,17,25]]
...
raw_right_img_data = raw_right_img_data[[4,6,9,18,12,17,25]]
...
raw_arm_joint_data = raw_arm_joint_data[[4,6,9,18,12,17,25]]
...
raw_hand_cmd_data = raw_hand_cmd_data[[4,6,9,18,12,17,25]]
...
raw_press_data = raw_press_data[[4,6,9,18,12,17,25]]
...
for t in range(T-1):
    ...
    [left_ksrnn_state, right_ksrnn_state, vsrnn_state, psrnn_state, urnn_state] = states
...
#     # plt.scatter(pca_val[i,:tg[0],0], pca_val[i,:tg[0],1])
#     # plt.scatter(pca_val[i,tg[0]:to[0],0], pca_val[i,tg[0]:to[0],1])
#     # plt.scatter(pca_val[i,to[0]:,0], pca_val[i,to[0]:,1])
#     plt.scatter(pca_val[i,:,0], pca_val[i,:,1], color=c)
# plt.scatter(pca_val[:,0,0], pca_val[:,0,1], color="k")
# pca_ratio = pca.explained_variance_ratio_ * 100
# plt.xlabel("PC1 ({:.1f}%)".format(pca_ratio[0]))
# plt.ylabel("PC2 ({:.1f}%)".format(pca_ratio[1]))
# plt.savefig("./fig/_pca_2d.png")
...
def anim_update(i):
    ...
    for n in range(left_img_data.shape[0]):
        ax.scatter(
...
ani.save(f'./output/PCA_StereoHSARNN_{params["tag"]}.gif')
