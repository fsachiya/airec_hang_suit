...
class MSA(nn.Module):
    def __init__(self, att_num, img_h, img_w, temperature, device, type='concat'):
        ...
        self.device = device
        self.type = type
        self.conv1 = nn.Sequential(nn.Conv2d(3, 16, 3, 1, padding=1, padding_mode='replicate'), nn.BatchNorm2d(16)) # 16, 128, 128
        self.conv2 = nn.Sequential(nn.Conv2d(16, 32, 3, 1, padding=1, padding_mode='replicate'), nn.BatchNorm2d(32)) # 32, 128, 128
        self.conv3 = nn.Sequential(nn.Conv2d(32, 64, 3, 1, padding=1, padding_mode='replicate'), nn.BatchNorm2d(64)) # 64, 64, 64

        self.att_weight1 = nn.Sequential(nn.Conv2d(16, att_num, 1, 1, bias=False, padding_mode='replicate'), nn.BatchNorm2d(att_num))
        self.att_weight2 = nn.Sequential(nn.Conv2d(32, att_num, 1, 1, bias=False, padding_mode='replicate'), nn.BatchNorm2d(att_num))
        self.att_weight3 = nn.Sequential(nn.Conv2d(64, att_num, 1, 1, bias=False, padding_mode='replicate'), nn.BatchNorm2d(att_num))

        self.att_feat = nn.Sequential(nn.Conv2d(3, att_num, 1, 1, bias=False, padding_mode='replicate'), nn.BatchNorm2d(att_num))

        if type == 'concat':
            self.sum_att = nn.Sequential(nn.Conv2d(att_num*3, att_num, 1, 1, bias=False, padding_mode='replicate'), nn.BatchNorm2d(att_num))
        if type == 'add':
        ...
        self.coord = coord.reshape(img_h * img_w, 2).to(device) # [16384, 2] -> 128,128,2
    ...
    def forward(self, img):    #  state
        feat = self.encoder(img)  # , state , gate_weights  # feat: 4, 8, 128, 128
        # att_map = self.get_attention_map(feat)  # att_map: 4, 8, 1, 16384
        # pt = self.get_pt(att_map).reshape(-1, self.att_num*2)   # pt: 4,16
        _pt = self.custom_get_pt(feat)
        pt = _pt.flatten(1,2)
    ...
    def get_attention_map(self, img):   # 4, 8, 128, 128
        ...
        att_map = att_map.unsqueeze(-2) # 4, 8, 1, 16384
        return att_map #b, n, 1, hw   # att_map: 4, 8, 1, 16384
    
    def custom_get_pt(self, img):
        img_size = 128
        _out = self.softmax2d(img)
    ...
    def get_pt(self, att_map):
        coord = self.coord.expand(att_map.shape[0], self.att_num, self.img_h*self.img_w, 2) # 4,8,16384,2 -> 4,8,128,128,2
        ij_coord = torch.matmul(att_map, coord) # att_map: 4,8,1,16384, coord: 4,8,16384,2 -> 4,8,1,2
...
class MSARNN(nn.Module):
    def __init__(self,
        ...
        self.rnn = nn.LSTMCell(self.att_num*2+self.robot_dim, 50)
