...
parser.add_argument("--state", type=str, choices=["k","v","p","u","i", "kc", "vc", "pc", "uc"], default="u")  # "f","fc", 
...
states = None
step = 500 if args.idx == -1 else args.idx*10   # 100
ksrnn_state_list, vsrnn_state_list, psrnn_state_list, urnn_state_list = [], [], [], []  # fsrnn_state_list, , []
ksrnn_cell_list, vsrnn_cell_list, psrnn_cell_list, urnn_cell_list = [], [], [], []  # fsrnn_cell_list, , []
img_size = params["img_size"]
T = x_right_img.shape[1]
for t in range(T-1):
    # predict rnn
    [yri_hat, yv_hat, yp_hat, right_enc_pts, right_dec_pts, states] = model(
        x_right_img[:, t], x_vec[:, t], x_press[:, t], step=step, states=states # with step
    )
    [ksrnn_state, vsrnn_state, psrnn_state, urnn_state] = states # psrnn_state, fsrnn_state, 
    # print(yri_hat.max(), yv_hat.max(), yp_hat.max(), right_enc_pts.max(), right_dec_pts.max())
    # fsrnn_state_list.append(fsrnn_state[0])
    ksrnn_state_list.append(ksrnn_state[0])
    vsrnn_state_list.append(vsrnn_state[0])
    psrnn_state_list.append(psrnn_state[0])
    urnn_state_list.append(urnn_state[0])
    # fsrnn_cell_list.append(fsrnn_state[1])
    ksrnn_cell_list.append(ksrnn_state[1])
    vsrnn_cell_list.append(vsrnn_state[1])
    psrnn_cell_list.append(psrnn_state[1])
    urnn_cell_list.append(urnn_state[1])

# fsrnn_states = torch.permute(torch.stack(fsrnn_state_list), (1, 0, 2))    # 207*7*20
# fsrnn_states = tensor2numpy(fsrnn_states)
ksrnn_states = torch.permute(torch.stack(ksrnn_state_list), (1, 0, 2))    # 207*7*20
ksrnn_states = tensor2numpy(ksrnn_states)
...
# fsrnn_cells = torch.permute(torch.stack(fsrnn_cell_list), (1, 0, 2))    # 207*7*20
# fsrnn_cells = tensor2numpy(fsrnn_cells)
ksrnn_cells = torch.permute(torch.stack(ksrnn_cell_list), (1, 0, 2))    # 207*7*20
ksrnn_cells = tensor2numpy(ksrnn_cells)
...
if args.state == "u":
    pass
# elif args.state == "f":
#     urnn_states = fsrnn_states
elif args.state == "k":
    urnn_states = ksrnn_states
elif args.state == "v":
    urnn_states = vsrnn_states
elif args.state == "p":
    urnn_states = psrnn_states
# elif args.state == "fc":
#     urnn_states = fsrnn_cells
elif args.state == "kc":
    urnn_states = ksrnn_cells
elif args.state == "vc":
