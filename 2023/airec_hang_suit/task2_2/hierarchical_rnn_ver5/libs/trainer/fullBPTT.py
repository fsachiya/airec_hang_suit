            ...
            for n_batch, ((xi, xv, xp), (yi, yv, yp)) in enumerate(data):   # x_joint, y_joint                
                ...
                with torch.cuda.amp.autocast(enabled=False):
                    ...
                    for t in range(T - 1):
                        ...
                        # if t == 0:
                        #     plt.figure()
                        #     plt.imshow(xi[0,0].permute(1,2,0).detach().clone().cpu().numpy())
                        #     plt.savefig(f"./fig/_sample_input_{step}.png")
                        #     plt.close()
                        
                        yi_hat, yv_hat, yp_hat, enc_pts, dec_pts, yf, states, heatmap, select_pts = self.model(
                            xi[:, t], xv[:, t], xp[:, t], states, prev_heatmap, prev_select_pts, prev_srnn_hid, t, step
                        )   # step is variable
                        # hrnn_e_time = time.time()
                        # print(hrnn_e_time - hrnn_s_time)
                        
                        prev_heatmap = heatmap.detach().clone()
                        prev_select_pts = select_pts.detach().clone()
                        prev_srnn_hid = torch.cat([states[1][0], states[2][0]], dim=1).detach().clone()   #.detach().clone()
                    ...
                    # _xf_data =  torch.permute(torch.stack(_xf_list), (1, 0, 2, 3, 4))
                    ...
                    # plt.figure()
                    # for i in range(32):
                    #     plt.imshow(_xf_data[0,0,i].clone().detach().numpy())
                    #     plt.savefig(f"./fig/msa_feat2/_xf_feat_{i}.png")
                    # ipdb.set_trace()
                    
                    # ipdb.set_trace()
                    # for i in range(32):
                    #     plt.scatter(_enc_pts_data[0,:,i,0].detach().clone().cpu().numpy(),
                    #     _enc_pts_data[0,:,i,1].detach().clone().cpu().numpy())
                    # plt.savefig("./fig/grid_feat/enc_scatter.png")
                    ###########
                    ...
                    # plt.figure()
                    # plt.plot(attn_frame_map[:,:,0].T.clone().detach().cpu().numpy())
                    # plt.savefig(f"./fig/attn_frame_map_{step}.png")
                    # ipdb.set_trace()
                    ...
                    # roll_pt_loss = self.criterion(roll_enc_pts_data, roll_dec_pts_data) * self.loss_w_dic["k"]
                    # delta_enc_pt_loss = self.criterion(enc_pts_data, roll_enc_pts_data) * self.loss_w_dic["k"]
                    # delta_dec_pt_loss = self.criterion(dec_pts_data, roll_dec_pts_data) * self.loss_w_dic["k"]
                    ...
                    # loss = img_loss + vec_loss + press_loss + roll_pt_loss + delta_pt_loss + state_var_loss + cell_var_loss + key_var_loss# + key_var_loss
                    # loss = modality_loss - key_var_loss*0.1    # + key_var_loss + state_var_loss + cell_var_loss + seq_state_var_loss
                ...
                if training:
                    ...
                    # self.optimizer.zero_grad(set_to_none=True)
                    # self.scaler.scale(loss.to(self.device)).backward()
                    # del loss
                ...
                # plt.figure()
                # plt.plot(yp[0,1:].clone().detach().cpu().numpy())
                # plt.plot(yp_hat_data[0].clone().detach().cpu().numpy())
                # plt.savefig(f"./fig/pred_press_step{step}.png")
                # plt.cla()
                # plt.clf()
                # plt.close()
                # # ipdb.set_trace()
                ...
                # plt.figure()
                # plt.plot(yv[0,1:].clone().detach().cpu().numpy())
                # plt.plot(yv_hat_data[0].clone().detach().cpu().numpy())
                # plt.savefig(f"./fig/pred_vector_step{step}.png")
                # plt.cla()
                # plt.clf()
                # plt.close()
                # ipdb.set_trace()
...
# 27.907851934432983, 27.158348083496094
# 35.03527855873108, 33.24407601356506