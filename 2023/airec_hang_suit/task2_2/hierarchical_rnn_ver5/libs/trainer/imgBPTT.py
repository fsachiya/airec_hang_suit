...
class imgBPTTtrainer:
    ...
    def process_epoch(self, data, epoch, training=True):   # epoch is variable
        with set_detect_anomaly(False):
            ...
            total_loss = 0.0
            for n_batch, ((xi, ), (yi, )) in enumerate(data):   # x_joint, y_joint
                ...
                with torch.cuda.amp.autocast(enabled=False):
                    ...
                    for t in range(T):
                        ...
                        yi_hat, enc_xf_last, dec_xfs, pts = self.model(
                    ...
                    img_loss = self.criterion(yi_hat_data, yi.to("cpu"))    # * self.loss_w_dic["i"] # [:, 1:]
                    # key_loss = self.criterion(enc_pts_data, dec_pts_data) * self.loss_w_dic["k"]
                    
                    modality_loss = img_loss    # + key_loss    # + roll_pt_loss + delta_pt_loss
                    print("modality", 
                        img_loss.item(), 
                        # key_loss.item(),
                        # roll_pt_loss.item(), 
