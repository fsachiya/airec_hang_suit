...
class MSAHSARNN3(nn.Module):
    def __init__(
        ...
        super(MSAHSARNN3, self).__init__()
        ...
        self.attn_key2heatmap = nn.Sequential(
            InverseSpatialSoftmax(
                width=self.img_size[0],
                height=self.img_size[1],
                heatmap_size=self.heatmap_size*0.1,
                normalized=True,
            )
        )
        ##################################################
        ...
        self.deep_encoder = DeepEncoder(self.key_dim, self.img_size)
        
        self.deep_cbam_encoder = DeepCBAMEncoder(self.key_dim, self.img_size)
        
        self.tdattn_deep_cbam_encoder = TopDownAttnDeepCBAMEncoder(self.key_dim, 
                                                                   self.img_size,
                                                                   srnn_hid_dim, 
                                                                   self.heatmap_size)
    ...
    def forward(self, xi, xv, xp, states=None, prev_heatmap=None, prev_select_pts=None, prev_hid=None, time=0, epoch=0):
        # print(time)
        
        
        # _, xf = cp.checkpoint(self.img_cbam_encoder, xi, use_reentrant=False)
        _, xf = cp.checkpoint(self.deep_cbam_encoder, xi, use_reentrant=False)
        # _xf, _enc_pts = cp.checkpoint(self.msa, xi)
        
        _enc_pts, _xf = cp.checkpoint(self.tdattn_deep_cbam_encoder, xi, prev_hid, time, use_reentrant=False)
        # _enc_pts, _xf = cp.checkpoint(self.deep_cbam_encoder, xi, use_reentrant=False)
        ...
        return yi, yv, yp, enc_pts, dec_pts, yf, states, prev_heatmap, prev_select_pts
...
class CBAMEncoder(nn.Module):
    ...
    def forward(self, xi):
        ...
        pad_x = torch.zeros_like(_out)
        pad_x[:,:,3:-3, 3:-3] = _out[:,:,3:-3, 3:-3]
...
class DeepEncoder(nn.Module):
    def __init__(self, key_dim, img_size
        ...
        self.conv1 = nn.Conv2d(3, 16, 3, 1, 1, padding_mode='replicate')
        self.conv2 = nn.Conv2d(16, 16, 3, 1, 1, padding_mode='replicate')
        self.conv3 = nn.Conv2d(16, 32, 3, 1, 1, padding_mode='replicate')
        self.conv4 = nn.Conv2d(32, 32, 3, 1, 1, padding_mode='replicate')
        self.conv5 = nn.Conv2d(32, self.key_dim, 3, 1, 1, padding_mode='replicate')
    ...
    def forward(self, xi):
        ...
        _flatten =pad_x.flatten(2,3)
        idx = _flatten.argmax(dim=2)
        coord_y = idx // self.img_size[0]
        coord_x = idx % self.img_size[0]
        coord_y = coord_y/self.img_size[0]
        coord_x = coord_x/self.img_size[0]
        pts = torch.stack((coord_x,coord_y)).permute((1,2,0))
...
class DeepCBAMEncoder(nn.Module):
    def __init__(self, key_dim, img_size
                 ):
        super(DeepCBAMEncoder, self).__init__()
        ...
        self.ca1 = ChannelAttention(in_planes=16, ratio=4)
        self.sa1 = SpatialAttention()
        self.ca2 = ChannelAttention(in_planes=16, ratio=4)
        self.sa2 = SpatialAttention()
        self.ca3 = ChannelAttention(in_planes=32, ratio=4)
        self.sa3 = SpatialAttention()
        self.ca4 = ChannelAttention(in_planes=32, ratio=4)
        self.sa4 = SpatialAttention()
        self.ca5 = ChannelAttention(in_planes=self.key_dim, ratio=4)
        ...
        self.bn5 = nn.BatchNorm2d(self.key_dim)
    ...
    def forward(self, xi):
        ...
        # heatmap = self.softmax2d(out)
        # attention_points = self.top_k_attention_points(heatmap, k=5)
...
class TopDownAttnDeepCBAMEncoder(nn.Module):
    def __init__(self, key_dim, img_size, srnn_hid_dim, heatmap_size
                 ):
        super(TopDownAttnDeepCBAMEncoder, self).__init__()
        ...
        self.conv1 = nn.Conv2d(5, 16, 3, 1, 1, padding_mode='replicate')
        self.conv2 = nn.Conv2d(16, 16, 3, 1, 1, padding_mode='replicate')
        self.conv3 = nn.Conv2d(16, 32, 3, 1, 1, padding_mode='replicate')
        self.conv4 = nn.Conv2d(32, 32, 3, 1, 1, padding_mode='replicate')
        self.conv5 = nn.Conv2d(32, self.key_dim, 3, 1, 1, padding_mode='replicate')
        ...
        ####################################
        # channel top-down attention
        self.td_channel_attn_layer1 = nn.Sequential(
            nn.Linear(self.srnn_hid_dim*2, 8, bias=False), 
            self.activation,
            nn.Linear(8, 16, bias=False), 
            nn.BatchNorm1d(16),
            nn.Sigmoid()
        )
        self.td_channel_attn_layer2 = nn.Sequential(
            nn.Linear(self.srnn_hid_dim*2, 16, bias=False), 
            self.activation,
            nn.Linear(16, 32, bias=False), 
            nn.BatchNorm1d(32),
            nn.Sigmoid()
        )
        self.td_channel_attn_layer3 = nn.Sequential(
            nn.Linear(self.srnn_hid_dim*2, self.key_dim//2, bias=False), 
            self.activation,
            nn.Linear(self.key_dim//2, self.key_dim, bias=False), 
            nn.BatchNorm1d(self.key_dim),
            nn.Sigmoid()
        )
        ####################################
        # spatial top-down attention
        self.td_spatial_attn_layer1 = nn.Sequential(
            nn.Linear(self.srnn_hid_dim*2, 16*2, bias=False), 
            nn.Sigmoid(),
            nn.Unflatten(1,(16,2)),
            InverseSpatialSoftmax(
                width=self.img_size[0],
        ...
        self.td_spatial_attn_layer2 = nn.Sequential(
            nn.Linear(self.srnn_hid_dim*2, 32*2, bias=False), 
            nn.Sigmoid(),
            nn.Unflatten(1,(32,2)),
            InverseSpatialSoftmax(
                width=self.img_size[0],
        ...
        self.td_spatial_attn_layer3 = nn.Sequential(
            nn.Linear(self.srnn_hid_dim*2, self.key_dim*2, bias=False), 
            nn.Sigmoid(),
            nn.Unflatten(1,(self.key_dim,2)),
            InverseSpatialSoftmax(
                width=self.img_size[0],
                height=self.img_size[1],
                heatmap_size=self.heatmap_size,
            ...
            nn.Conv2d(self.key_dim,self.key_dim//2,3,1,1),
            self.activation,
            nn.Conv2d(self.key_dim//2,self.key_dim,3,1,1),
    ...
    def forward(self, xi, prev_hid, time):
        x_coords = torch.linspace(0, 1, steps=self.img_size[0]).unsqueeze(0).repeat(self.img_size[0], 1)
        x_coords = nn.Unflatten(0,(1,1,128))(x_coords).repeat(xi.shape[0], 1, 1, 1).to(xi.device)
        y_coords = torch.linspace(-1, 1, steps=self.img_size[0]).unsqueeze(1).repeat(1, self.img_size[0])
        y_coords = nn.Unflatten(0,(1,1,128))(y_coords).repeat(xi.shape[0], 1, 1, 1).to(xi.device)
        xi = torch.cat([xi, x_coords, y_coords], dim=1)
        
        # channel top-down attention
        # 16, 32, key_dim
        if prev_hid == None:
            prev_hid = torch.rand(xi.shape[0], self.srnn_hid_dim*2).to(xi.device)
            prev_td_c_attn_w1 = nn.Unflatten(1,(16,1,1))(self.td_channel_attn_layer1(prev_hid))
            prev_td_c_attn_w2 = nn.Unflatten(1,(32,1,1))(self.td_channel_attn_layer2(prev_hid))
            prev_td_c_attn_w3 = nn.Unflatten(1,(self.key_dim,1,1))(self.td_channel_attn_layer3(prev_hid))
            # prev_td_c_attn_w1 = torch.rand(xi.shape[0], 16, 1, 1).to(xi.device)
            # prev_td_c_attn_w2 = torch.rand(xi.shape[0], 32, 1, 1).to(xi.device)
            # prev_td_c_attn_w3 = torch.rand(xi.shape[0], self.key_dim, 1, 1).to(xi.device)
        else:
            prev_td_c_attn_w1 = nn.Unflatten(1,(16,1,1))(self.td_channel_attn_layer1(prev_hid))
            prev_td_c_attn_w2 = nn.Unflatten(1,(32,1,1))(self.td_channel_attn_layer2(prev_hid))
            prev_td_c_attn_w3 = nn.Unflatten(1,(self.key_dim,1,1))(self.td_channel_attn_layer3(prev_hid))
        ...
        if prev_hid == None:
            prev_hid = torch.rand(xi.shape[0], self.srnn_hid_dim*2).to(xi.device)
            prev_td_s_attn_w1 = self.td_spatial_attn_layer1(prev_hid)
            prev_td_s_attn_w2 = self.td_spatial_attn_layer2(prev_hid)
            prev_td_s_attn_w3 = self.td_spatial_attn_layer3(prev_hid)
            # _pts1 = nn.Sigmoid()(torch.rand(xi.shape[0], 16, 2).to(xi.device))
            # prev_td_s_attn_w1 = self.key2heatmap(_pts1)
            # _pts2 = nn.Sigmoid()(torch.rand(xi.shape[0], 32, 2).to(xi.device))
            # prev_td_s_attn_w2 = self.key2heatmap(_pts2)
            # _pts3 = nn.Sigmoid()(torch.rand(xi.shape[0], self.key_dim, 2).to(xi.device))
            # prev_td_s_attn_w3 = self.key2heatmap(_pts3)
        else:
            prev_td_s_attn_w1 = self.td_spatial_attn_layer1(prev_hid)
            prev_td_s_attn_w2 = self.td_spatial_attn_layer2(prev_hid)
            prev_td_s_attn_w3 = self.td_spatial_attn_layer3(prev_hid)
            # _pts1 = nn.Sigmoid()(self.td_spatial_attn_layer1(prev_hid)).reshape(-1, 16, 2)
            # prev_td_s_attn_w1 = self.key2heatmap(_pts1)
            # _pts2 = nn.Sigmoid()(self.td_spatial_attn_layer2(prev_hid)).reshape(-1, 32, 2)
            # prev_td_s_attn_w2 = self.key2heatmap(_pts2)
            # _pts3 = nn.Sigmoid()(self.td_spatial_attn_layer3(prev_hid)).reshape(-1, self.key_dim, 2)
            # prev_td_s_attn_w3 = self.key2heatmap(_pts3)
        ...
        out = self.bn1(out)
        out = self.activation(out)
        out = torch.mul(out, prev_td_c_attn_w1)
        # out = out + prev_td_s_attn_w1
        out = torch.mul(out, prev_td_s_attn_w1)
        ...
        #         plt.savefig(f"./fig/td_sc_mid_attn/out_feat_lay1_{time}_{i}.png")
        #         plt.close()
        ...
        out = self.bn3(out)
        out = self.activation(out)
        out = torch.mul(out, prev_td_c_attn_w2)
        # out = out + prev_td_s_attn_w2
        out = torch.mul(out, prev_td_s_attn_w2)
        ...
        #         plt.figure()
        #         plt.imshow(out[0,i].detach().clone().cpu().numpy())
        #         plt.savefig(f"./fig/td_sc_mid_attn/out_feat_lay2_{time}_{i}.png")
        #         plt.close()
        ...
        out = self.bn5(out)
        out = self.activation(out)
        out = torch.mul(out, prev_td_c_attn_w3)
        # out = out + prev_td_s_attn_w3
        out = torch.mul(out, prev_td_s_attn_w3)
        ...
        #         plt.figure()
        #         plt.imshow(out[0,i].detach().clone().cpu().numpy())
        #         plt.savefig(f"./fig/td_sc_mid_attn/out_feat_lay3_{time}_{i}.png")
        #         plt.close()
        ...
        # _out = torch.mul(out, prev_td_channel_attn_weight)
        # _out = torch.mul(out, prev_td_spatial_attn_weight)
        
        # if prev_tdattn_weight == None:
        #     prev_tdattn_weight = 1.0
        #     _out = torch.mul(out, prev_tdattn_weight)
        # else:
        #     prev_tdattn_weight = prev_tdattn_weight.unsqueeze(-1).unsqueeze(-1)
        #     _prev_tdattn_weight = torch.where(prev_tdattn_weight < 0.0, 0.0, prev_tdattn_weight)
        #     _out = torch.mul(out, prev_tdattn_weight)
        #     _selective_out = torch.mul(out, _prev_tdattn_weight)
        
        sum_out = out.sum(dim=1)
        sum_softmax_out = self.softmax2d(sum_out)
            
        # plt.figure()
        # plt.imshow(sum_softmax_out[0].detach().clone().cpu().numpy())
        # plt.savefig(f"./fig/td_sc_mid_attn/sum_softmax_out_feat_{time}.png")
        # plt.close()
        
        _out = self.softmax2d(out)
        
        pad_x = torch.ones_like(_out)*_out.min()
        pad_x[:,:,3:-3, 3:-3] = _out[:,:,3:-3, 3:-3]
        _flatten =pad_x.flatten(2,3)
        idx = _flatten.argmax(dim=2)
        coord_y = idx // self.img_size[0]
...
#         min_val = heatmap.view(heatmap.size(0), heatmap.size(1), -1).min(dim=-1, keepdim=True)[0]
#         max_val = heatmap.view(heatmap.size(0), heatmap.size(1), -1).max(dim=-1, keepdim=True)[0]
#         min_val = min_val.view(min_val.size(0), min_val.size(1), 1, 1)
#         max_val = max_val.view(max_val.size(0), max_val.size(1), 1, 1)
#         normalized_heatmap = (heatmap - min_val) / (max_val - min_val)
#         return normalized_heatmap
    
#     def nms(self, heatmap, threshold=0.5):
#         """Non-Maximum Suppression to extract attention points."""
#         # Apply threshold to heatmap
#         mask = heatmap > threshold
#         heatmap = heatmap * mask.float()

#         # Extract indices of non-zero elements
#         indices = torch.nonzero(heatmap, as_tuple=False)
#         batch_indices = indices[:, 0]
#         channel_indices = indices[:, 1]
#         spatial_indices = indices[:, 2:]

#         values = heatmap[batch_indices, channel_indices, spatial_indices[:, 0], spatial_indices[:, 1]]

#         # Sort the indices by their values
#         sorted_indices = torch.argsort(values, descending=True)
#         selected_indices = {
#             'batch': batch_indices[sorted_indices],
#             'channel': channel_indices[sorted_indices],
#             'spatial': spatial_indices[sorted_indices]
#         }
#         return selected_indices
    
#     def top_k_attention_points(self, heatmap, k=5):
#         """Extract top-k attention points from the heatmap."""
#         # Normalize heatmap to consider relative brightness
#         normalized_heatmap = self.normalize_heatmap(heatmap)

#         # Apply NMS to get multiple attention points
#         attention_points = self.nms(normalized_heatmap, threshold=0.5)

#         # Select top-k points for each batch and channel
#         top_k_indices = {
#             'batch': [],
#             'channel': [],
#             'spatial': []
...
#         unique_batches = torch.unique(attention_points['batch'])
#         for batch in unique_batches:
#             batch_mask = attention_points['batch'] == batch
#             unique_channels = torch.unique(attention_points['channel'][batch_mask])
#             for channel in unique_channels:
#                 channel_mask = (attention_points['batch'] == batch) & (attention_points['channel'] == channel)
#                 if torch.sum(channel_mask) > k:
#                     channel_indices = {
#                         'batch': attention_points['batch'][channel_mask][:k],
#                         'channel': attention_points['channel'][channel_mask][:k],
#                         'spatial': attention_points['spatial'][channel_mask][:k]
#                     }
#                 else:
#                     channel_indices = {
#                         'batch': attention_points['batch'][channel_mask],
#                         'channel': attention_points['channel'][channel_mask],
#                         'spatial': attention_points['spatial'][channel_mask]
#                     }
#                 top_k_indices['batch'].append(channel_indices['batch'])
#                 top_k_indices['channel'].append(channel_indices['channel'])
#                 top_k_indices['spatial'].append(channel_indices['spatial'])
                
#         top_k_indices['batch'] = torch.cat(top_k_indices['batch'])
#         top_k_indices['channel'] = torch.cat(top_k_indices['channel'])
#         top_k_indices['spatial'] = torch.cat(top_k_indices['spatial'])
...
class HeatCBAMEncoder(nn.Module):
    def __init__(self, key_dim, img_size):
        ...
        self.conv1 = nn.Conv2d(3, 16, 3, 1, 1, bias=False, padding_mode='replicate')
        self.conv2 = nn.Conv2d(16, 32, 3, 1, 1, bias=False, padding_mode='replicate')
        self.conv3 = nn.Conv2d(32, self.key_dim, 3, 1, 1, bias=False, padding_mode='replicate')
    ...
    def forward(self, xi, prev_heatmap, prev_select_pts, time, epoch):
        ...
        for i in range(16):
            ...
            plt.savefig(f"./fig/td_sc_attn4/out_feat_{time}_{i}.png")
            plt.close()
        ...
        for i in range(8):
            plt.figure()
            plt.imshow(out[0,i].detach().clone().cpu().numpy())
            plt.savefig(f"./fig/cbam/out_feat_{i}.png")
            plt.close()
...
# plt.imshow(yi_hat[0].permute(1,2,0).detach().clone().cpu().numpy(), origin='upper')
# plt.scatter(pts[0,:,0], pts[0,:,1])
# plt.scatter(select_pts[0,:,0], select_pts[0,:,1])
# plt.savefig(f"./fig/abs/scatter_plot.png")
# plt.close()
...
for i in range(8):
    plt.figure()
    plt.imshow(selective_sum[0].detach().clone().cpu().numpy())
    plt.savefig(f"./fig/td_attn4/selective_sum.png")
    plt.close()
