...
class MSAHSARNN(nn.Module):
    ...
    def forward(self, xi, xv, xp, states=None, prev_heatmap=None, prev_select_pts=None, time=0, epoch=0):
        # xf = cp.checkpoint(self.img_encoder, xi)
        # if time == 0:
        #     plt.figure()
        #     plt.imshow(xi[0].permute(1,2,0).detach().clone().cpu().numpy())
        #     plt.savefig(f"./fig/sample_input_{epoch}.png")
        #     plt.close()
...
class HeatCBAMEncoder(nn.Module):
    ...
    def forward(self, xi, prev_heatmap, prev_select_pts, time, epoch):
        ...
        for i in range(8):
            ...
            plt.savefig(f"./fig/kmeans_feat/_out_feat_{i}.png")
            plt.close()
