...
class MSAHSARNN3(nn.Module):
    def __init__(
        ...
        self.tdattn_deep_cbam_encoder = TopDownAttnDeepCBAMEncoder(self.key_dim, self.img_size)
        ...
        self.top_down_attn_weight = nn.Sequential(
            nn.Linear(urnn_hid_dim, self.key_dim, bias=False), 
    ...
    def forward(self, xi, xv, xp, states=None, prev_heatmap=None, prev_select_pts=None, prev_tdattn_weight=None, time=0, epoch=0):
        ...
        _enc_pts, _xf = cp.checkpoint(self.tdattn_deep_cbam_encoder, xi, prev_tdattn_weight, use_reentrant=False)
        ...
        prev_tdattn_weight = self.top_down_attn_weight(states[3][0])
        ...
        return yi, yv, yp, enc_pts, dec_pts, yf, states, prev_heatmap, prev_select_pts, prev_tdattn_weight
...
class DeepCBAMEncoder(nn.Module):
    def __init__(self, key_dim, img_size
        ...
        self.conv3 = nn.Conv2d(16, 32, 3, 1, 1, padding_mode='replicate')
        self.conv4 = nn.Conv2d(32, 32, 3, 1, 1, padding_mode='replicate')
        self.conv5 = nn.Conv2d(32, self.key_dim, 3, 2, 1, padding_mode='replicate')
...
class TopDownAttnDeepCBAMEncoder(nn.Module):
    def __init__(self, key_dim, img_size
        ...
        self.conv4 = nn.Conv2d(32, 32, 3, 2, 1, padding_mode='replicate')
        self.conv5 = nn.Conv2d(32, self.key_dim, 3, 1, 1, padding_mode='replicate')
    ...
    def forward(self, xi, prev_tdattn_weight):
        # out = cp.checkpoint(self.conv1, xi)
        out = self.conv1(xi)
        residual = out
        out = self.ca1(out) * out
        ...
        sum_out = _out.sum(dim=1)
        sum_softmax_out = self.softmax2d(sum_out)
        
        
        ipdb.set_trace()
        
        _out = self.softmax2d(_out)
...
class HeatCBAMEncoder(nn.Module):
    ...
    def forward(self, xi, prev_heatmap, prev_select_pts, time, epoch):
        ...
        for i in range(16):
            ...
            plt.savefig(f"./fig/td_attn5/out_feat_{i}.png")
            plt.close()
