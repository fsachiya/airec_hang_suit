...
class GHSARNN(nn.Module):
    def __init__(
        ...
        self.hrnn = HierarchicalRNNCell(
            srnn_input_dims={"k": 2*self.key_dim*self.grid_num, "v": vec_dim, "p": press_dim},    # *self.grid_num  self.overlaped_grid_num
        ...
        self.adaptive_grid_key_cbam_encoder = nn.Sequential(
            AdaptiveGridCBAMEncoder(self.img_size, 
                                        self.key_dim,
                                        self.heatmap_size*0.1)
        )
        ...
        self.key_decoder = nn.Sequential(
            nn.Linear(srnn_hid_dim, 2*self.key_dim*self.grid_num, bias=False),    # *self.grid_num    *self.overlaped_grid_num
        ...
        self.img_cbam_encoder = nn.Sequential(
            CBAMEncoder(self.key_dim*self.grid_num, self.img_size),   # *self.grid_num    *self.overlaped_grid_num
        )
        ...
        self.img_decoder = nn.Sequential(   # img_decoder
            nn.ConvTranspose2d(self.key_dim*self.grid_num, 32, 3, 1, 1),  # Transposed Convolutional layer 1  # *self.grid_num  self.overlaped_grid_num
            # nn.GroupNorm(1, 32),
            activation,
    ...
    def forward(self, xi, xv, xp, states=None):
        ...
        _enc_pts, _ = cp.checkpoint(self.adaptive_grid_key_cbam_encoder, xi, use_reentrant=False)
        ...
        _dec_pts = dec_pts.reshape(-1, self.key_dim*self.grid_num, 2) # *self.grid_num   self.overlaped_grid_num
...
class SimpleAdaptiveGridCBAMEncoder(nn.Module):
    def __init__(self, 
                 input_dim,
                 output_dim,
                 grid_side, 
        ...
        super(SimpleAdaptiveGridCBAMEncoder, self).__init__()
        self.input_dim = input_dim
        self.output_dim = output_dim
        # self.grid_num = grid_num
        self.grid_side = grid_side
    ...
    def forward(self, xi, pts):
        full_img = torch.zeros((xi.shape[0], self.output_dim, self.img_size[0], self.img_size[0])).to(xi.device)
        full_heatmap = torch.zeros((xi.shape[0], self.output_dim, self.img_size[0], self.img_size[0])).to(xi.device)
                
        exp_img = F.pad(xi, pad=[64,64,64,64], mode="replicate")
        
        # exp_img = torch.rand((xi.shape[0], xi.shape[1], self.img_size[0]+self.grid_size[0]*2, self.img_size[0]+self.grid_size[0]*2)).to(xi.device)
        # exp_img[:,:,self.grid_size[0]:self.grid_size[0]+self.img_size[0],self.grid_size[0]:self.grid_size[0]+self.img_size[0]] = xi
        # pts: 8, 16, 8, 2
        # xi: 8, 8, 128, 128
                
        # _ave_pts = ave_pts.mean(dim=0) * 128
        
        _pts = pts * 128
        _pts = _pts.round()
        exp_pts = _pts + self.grid_size[0]
        exp_lt_pts = exp_pts - int(self.grid_size[0]/2)
        
        _exp_lt_pts = exp_lt_pts.mean(dim=2).unsqueeze(dim=2).repeat(1, 1, 4, 1)
        
        # _lt_pts = _pts - int(self.grid_size[0]/2)
        # _lt_pts = _lt_pts.clip(0,128)
                
        grid_enc_pts_list = []
        grid_list = []
        for i in range(pts.shape[1]):     # int(self.grid_size[0]/2)
            _grid_list = []
            for j in range(pts.shape[0]):  # 8
                split_grid_list = []
                for k in range(pts.shape[2]):  # 8
                    exp_lt_pt_x = int(exp_lt_pts[j, i, k, 0].item())
                    exp_lt_pt_y = int(exp_lt_pts[j, i, k, 1].item())
                    
                    # hoge = xi[0,2, 11:self.grid_size[0]+11, 0:self.grid_size[0]]
                    # lt_pt_x = int(_lt_pts[j, i, k, 0].item())   # 0
                    # lt_pt_y = int(_lt_pts[j, i, k, 1].item())   # 14
                    
                    split_grid = exp_img[j, k, exp_lt_pt_y:exp_lt_pt_y+self.grid_size[0], exp_lt_pt_x:exp_lt_pt_x+self.grid_size[0]]
                    # split_grid = xi[j, k, lt_pt_y:lt_pt_y+self.grid_size[0], lt_pt_x:lt_pt_x+self.grid_size[0]]                    
                    split_grid_list.append(split_grid)
                    
                split_grid_data = torch.stack(split_grid_list)
                _grid_list.append(split_grid_data)
            
            grid = torch.stack(_grid_list)
            
            _grid_enc_pts, _grid = self.simple_grid_cbam_encoder(grid)
            
            _grid_enc_pts = _grid_enc_pts * self.grid_size[0]   # exy
            _grid_enc_pts[:,:,0] += exp_lt_pts[:,i,:,0] # ex'
            _grid_enc_pts[:,:,1] += exp_lt_pts[:,i,:,1] # ey'
            
            _grid_enc_pts -= self.grid_size[0]
            _grid_enc_pts = _grid_enc_pts.clip(0,128)
...
class AdaptiveGridCBAMEncoder(nn.Module):
    def __init__(self, 
                #  grid_num,
                #  grid_side, 
                #  grid_size, 
        ...
        super(AdaptiveGridCBAMEncoder, self).__init__()
        # self.grid_num = grid_num
        # self.grid_side = grid_side
        # self.grid_size = grid_size
        self.img_size = img_size
        ...
        self.simple_grid_cbam_encoder1 = SimpleGridCBAMEncoder(3, self.key_dim, 
                                                               self.grid_side1, self.grid_size1, img_size, self.heatmap_size)
        self.simple_grid_cbam_encoder2 = SimpleGridCBAMEncoder(self.key_dim, self.key_dim, 
                                                               self.grid_side2, self.grid_size2, img_size, self.heatmap_size)
        self.simple_grid_cbam_encoder3 = SimpleGridCBAMEncoder(32, self.key_dim, 
                                                               self.grid_side3, self.grid_size3, img_size, self.heatmap_size)
        
        self.simple_adaptive_grid_cbam_encoder = SimpleAdaptiveGridCBAMEncoder(self.key_dim, self.key_dim, 
                                                               self.grid_side1, self.grid_size1, img_size, self.heatmap_size)
    ...
    def forward(self, xi):
        ...
        # _xi = torch.cat([xi, x_coords, y_coords], dim=1)
        _pts, _grid, _full = self.simple_grid_cbam_encoder1(xi)
        # grid = _grid.reshape(_grid.shape[0], self.grid_side1**2, -1, _grid.shape[2], _grid.shape[3])
        ipdb.set_trace()
        _pts, _grid, _full = self.simple_grid_cbam_encoder2(_full)
        
        pts = _pts.reshape(_pts.shape[0], self.grid_side1**2, -1, _pts.shape[2])
        # ave_pts = pts.mean(dim=2)
        ipdb.set_trace()
        grid = _grid.reshape(_grid.shape[0], self.grid_side2**2, -1, _grid.shape[2], _grid.shape[3])
        grid_var = grid.var(dim=3).var(dim=3)
        grid_var_mean = grid_var.mean(dim=2)
        
        _pts, _grid = self.simple_adaptive_grid_cbam_encoder(_full, pts)
        
        # for i in range(16):
        #     plt.imshow(grid[0,i,0].detach().clone().cpu().numpy())
        #     plt.savefig(f"./fig/grid_feat2/grid_feat_{i}0_lay0.png")
        
        
        
        # max_val = grid.max(dim=3)[0].max(dim=3)[0]
        # min_val = grid.min(dim=3)[0].min(dim=3)[0]
        # ave_val = grid.mean(dim=[3,4])
        # threshold_upper = (max_val - min_val) * 0.9 + min_val
        # threshold_lower = (max_val - min_val) * 0.1 + min_val
        
        # threshold_upper = threshold_upper.unsqueeze(dim=-1).unsqueeze(dim=-1)
        # threshold_lower = threshold_lower.unsqueeze(dim=-1).unsqueeze(dim=-1)
        # threshold_average = ave_val.unsqueeze(dim=-1).unsqueeze(dim=-1)
        
        # updated_tensor = torch.where(
        #     grid > threshold_upper,   #  & (grid > threshold_lower)
        #     torch.tensor(1.0, device=grid.device),
        #     grid
        # )
        # updated_tensor = torch.where(
        #     updated_tensor < threshold_lower,   #  & (grid > threshold_lower)
        #     torch.tensor(0.0, device=updated_tensor.device),
        #     updated_tensor
        # )
