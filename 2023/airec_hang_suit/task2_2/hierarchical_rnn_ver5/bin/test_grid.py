...
try:
    from libs.model import GHSARNN
    from libs.utils import moving_average
except:
    sys.path.append("./libs/")
    from model import GHSARNN
...
if __name__ == "__main__":
    ...
    for t in range(T-1):
        ...
        try:
            ...
            _xv = xv[:, t]
        yi_hat, yv_hat, yp_hat, enc_pts, dec_pts, yf, states = model(
            xi[:, t], _xv, xp[:, t], states=states  # step=500, 
        )
        end_time = time.time()
    ...
    # plt_enc_pts_data = torch.permute(enc_pts_data[0], (1,0)).view(2,8,-1).detach().numpy()
    # plt_dec_pts_data = torch.permute(dec_pts_data[0], (1,0)).view(2,8,-1).detach().numpy()
    ...
    #     plt.plot(plt_roll_enc_pts_data[0,i,:], linestyle="dashed")
    # plt.savefig("./fig/enc_pts_plot_x.png")
    # plt.close()
    # plt.figure()
    # for i in range(plt_enc_pts_data.shape[1]):
    #     # plt.plot(plt_enc_pts_data[1,i,:])
    #     plt.plot(plt_roll_enc_pts_data[1,i,:], linestyle="dashed")
    ...
    #     plt.plot(plt_roll_dec_pts_data[0,i,:], linestyle="dashed")
    # plt.savefig("./fig/dec_pts_plot_x.png")
    # plt.close()
    # plt.figure()
    # for i in range(plt_dec_pts_data.shape[1]):
    #     # plt.plot(plt_dec_pts_data[1,i,:])
    #     plt.plot(plt_roll_dec_pts_data[1,i,:], linestyle="dashed")
    ...
    # attention points
    _enc_pts_data = enc_pts_data[0].reshape([-1, params["key_dim"]*4, 2]) # *4
    _dec_pts_data = dec_pts_data[0].reshape([-1, params["key_dim"]*4, 2]) # *4
    ...
    def anim_update(i):
        ...
        for j in range(params["key_dim"]*4):  # *4
