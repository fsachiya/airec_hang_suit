...
try:
    from libs.model import MSAHSARNN3
    from libs.utils import moving_average
except:
    sys.path.append("./libs/")
    from model import MSAHSARNN3
...
if __name__ == "__main__":
    ...
    for t in range(T-1):
        ...
        try:
            _xv = 1.0*xv[:, t] + 0.0*yv_hat_list[-1]
        except:
            _xv = xv[:, t]
        yi_hat, yv_hat, yp_hat, enc_pts, dec_pts, yf, states, heatmap, select_pts = model(
            xi[:, t], _xv, xp[:, t], states, prev_heatmap, prev_select_pts, prev_srnn_hid, t 
        )
        end_time = time.time()
        print("duration", end_time - start_time)
        
        prev_heatmap = heatmap
        prev_srnn_hid = torch.cat([states[1][0], states[2][0]], dim=1)
    ...
    # enc_pts_data = torch.cat((enc_pts_data, enc_pts_data[:,-1].view(1,1,-1)), dim=1)
    _dec_pts_data = torch.permute(torch.stack(dec_pts_list[:-1]), (1, 0, 2))
    dec_pts_data = torch.cat([_dec_pts_data, torch.unsqueeze(_dec_pts_data[:,0], dim=1)], dim=1)
    # dec_pts_data = torch.cat((dec_pts_data, dec_pts_data[:,-1].view(1,1,-1)), dim=1)
    # roll_enc_pts_data = torch.tensor(np.apply_along_axis(moving_average, 1, enc_pts_data.cpu().detach().numpy(), size=7)).to(enc_pts_data.device)
    # roll_dec_pts_data = torch.tensor(np.apply_along_axis(moving_average, 1, dec_pts_data.cpu().detach().numpy(), size=7)).to(dec_pts_data.device)
    plt_enc_pts_data = torch.permute(enc_pts_data[0], (1,0)).view(2,16,-1).detach().numpy()
    plt_dec_pts_data = torch.permute(dec_pts_data[0], (1,0)).view(2,16,-1).detach().numpy()
    # plt_roll_enc_pts_data = torch.permute(roll_enc_pts_data[0], (1,0)).view(2,8,-1).detach().numpy()
    # plt_roll_dec_pts_data = torch.permute(roll_dec_pts_data[0], (1,0)).view(2,8,-1).detach().numpy()

    # _urnn_hid = torch.permute(torch.stack(urnn_hid_list), (1,0,2))
    # plt.figure()
    # plt.plot(_urnn_hid[0].clone().detach().numpy())
    # plt.savefig("./fig/urnn_hid.png")
    # # ipdb.set_trace()

    ipdb.set_trace()
    _yi_hat_data = yi_hat_data.permute(0,1,3,4,2).detach().clone().cpu().numpy()
    enc_pts_data = enc_pts_data.reshape(-1, 207, 16, 2)
    _pts_data = enc_pts_data.detach().clone().cpu().numpy()
    _pts_data = (_pts_data*128).astype(int)
    for i in range(200):
        plt.figure()
        plt.imshow(_yi_hat_data[0,i], origin='upper')
        plt.scatter(_pts_data[0,i,:,0], _pts_data[0,i,:,1])
        plt.savefig(f"./fig/td_sc_mid_attn/scatter_plot_{i}.png")
        plt.close()
    ...
    ############################################
    # plt.figure()
    # for i in range(plt_enc_pts_data.shape[1]):
    #     plt.scatter(plt_enc_pts_data[0,i,:], plt_enc_pts_data[1,i,:])
    #     plt.plot(plt_enc_pts_data[0,i,:], plt_enc_pts_data[1,i,:])
    # plt.xlim(0,1)
    # plt.ylim(0,1)
    # plt.savefig("./fig/enc_pts_scatter.png")
    # plt.close()

    # plt.figure()
    # for i in range(plt_dec_pts_data.shape[1]):
    #     plt.scatter(plt_roll_dec_pts_data[0,i,:], plt_roll_dec_pts_data[1,i,:])
    #     plt.plot(plt_roll_dec_pts_data[0,i,:], plt_roll_dec_pts_data[1,i,:])
    # plt.xlim(0,1)
    # plt.ylim(0,1)
    # plt.savefig("./fig/dec_pts_scatter.png")
    # plt.close()

    # enc_pts_xy
    plt.figure()
    for i in range(plt_enc_pts_data.shape[1]):
        # plt.plot(plt_enc_pts_data[0,i,:])
        plt.plot(plt_enc_pts_data[0,i,:], linestyle="dashed")
    plt.savefig("./fig/enc_pts_plot_x.png")
    plt.close()
    plt.figure()
    for i in range(plt_enc_pts_data.shape[1]):
        # plt.plot(plt_enc_pts_data[1,i,:])
        plt.plot(plt_enc_pts_data[1,i,:], linestyle="dashed")
    plt.savefig("./fig/enc_pts_plot_y.png")
    plt.close()

    # dec_pts_xy
    plt.figure()
    for i in range(plt_dec_pts_data.shape[1]):
        # plt.plot(plt_dec_pts_data[0,i,:])
        plt.plot(plt_dec_pts_data[0,i,:], linestyle="dashed")
    plt.savefig("./fig/dec_pts_plot_x.png")
    plt.close()
    plt.figure()
    for i in range(plt_dec_pts_data.shape[1]):
        # plt.plot(plt_dec_pts_data[1,i,:])
        plt.plot(plt_dec_pts_data[1,i,:], linestyle="dashed")
    plt.savefig("./fig/dec_pts_plot_y.png")
    plt.close()
    ...
    # calc loss
    img_loss = (yi_hat_data - yi[:, 1:])**2 * loss_w_dic["i"]
    vec_loss = (yv_hat_data - yv[:, 1:])**2 * loss_w_dic["v"]
    press_loss = (yp_hat_data - yp[:, 1:])**2 * loss_w_dic["p"]

    key_loss = (enc_pts_data - dec_pts_data)**2 * loss_w_dic["k"]
    # roll_key_loss = (roll_enc_pts_data - roll_dec_pts_data)**2 * loss_w_dic["k"]

    # delta_enc_key_loss = (enc_pts_data - roll_enc_pts_data)**2 * loss_w_dic["k"]
    # delta_dec_key_loss = (dec_pts_data - roll_dec_pts_data)**2 * loss_w_dic["k"]

    img_loss = img_loss.mean(dim=(2,3,4))
    vec_loss = vec_loss.mean(dim=-1)
    press_loss = press_loss.mean(dim=-1)
    key_loss = key_loss.mean(dim=-1)
    # roll_key_loss = roll_key_loss.mean(dim=-1)
    # delta_enc_key_loss = delta_enc_key_loss.mean(dim=-1)
    # delta_dec_key_loss = delta_dec_key_loss.mean(dim=-1)
    # delta_key_loss = delta_enc_key_loss + delta_dec_key_loss
    loss = img_loss + vec_loss + press_loss + key_loss

    # # split key points
    # ect_pts = np.array(ect_pts_list)
    # dec_pts = np.array(dec_pts_list)
    # ect_pts = ect_pts.reshape(-1, params["key_dim"], 2) * img_size
    # dec_pts = dec_pts.reshape(-1, params["key_dim"], 2) * img_size
    ...
    _pred_arm_state_data = _pred_vec_data[:,:,:7]
    _pred_hand_cmd_data = _pred_vec_data[:,:,7:]
    pred_arm_state_data = normalization(_pred_arm_state_data, minmax, arm_state_bounds[:, 7:])
    ...
    # roll_enc_pts_data = roll_enc_pts_data.detach().numpy()
    # roll_dec_pts_data = roll_dec_pts_data.detach().numpy()

    # arange data for plot
    # img
    # ipdb.set_trace()
    plt_img_data = plt_img_data[0].astype(int)
    # plt_img_data = plt_img_data[idx,:,128:,128-32:256-32].astype(int)
    # _pred_img_data = pred_img_data.astype(int)
    plt_pred_img_data = pred_img_data[idx].astype(int)
    
    # state
    plt_arm_state_data = plt_arm_state_data[idx]
    plt_pred_arm_state_data = pred_arm_state_data[idx]

    # command
    plt_hand_cmd_data = plt_hand_cmd_data[idx]
    plt_pred_hand_cmd_data = pred_hand_cmd_data[idx]
    ...
    # ax[0].set_xlim(0, T)
    # ax[0].plot(plt_arm_state_data[1:], linestyle="dashed", c="k")
    # # right arm has 7 joints, not 8
    # for joint_idx in range(7):
    #     ax[0].plot(np.arange(T), plt_pred_arm_state_data[:T, joint_idx])
    ...
    # attention points
    _enc_pts_data = enc_pts_data[0].reshape([-1, params["key_dim"], 2]) # *4
    _dec_pts_data = dec_pts_data[0].reshape([-1, params["key_dim"], 2]) # *4
