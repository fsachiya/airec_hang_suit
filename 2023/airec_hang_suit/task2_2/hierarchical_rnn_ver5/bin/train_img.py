...
try:
    ...
    from libs.utils import ImgDataset
    from libs.utils import moving_average
    from libs.trainer import imgBPTTtrainer
except:
    ...
    from trainer import imgBPTTtrainer
...
if __name__ == '__main__':
    ...
    parser.add_argument("--heatmap_size", type=float, default=0.1) # 0.1
    parser.add_argument("--temperature", type=float, default=1e-4)  # 1e-4

    parser.add_argument("--stdev", type=float, default=0.1)
    parser.add_argument("--lr", type=float, default=0.001)   # 1e-3
    ...
    #==============================================#
    # make dataset
    train_dataset = ImgDataset(img_data, 
                                    device=device, 
    ...
    #==============================================#
    test_dataset = ImgDataset(img_data, 
                                    device=device, 
                                    dataset_device=device,
                                    stdev=stdev)
    ...
    with tqdm(range(args.epoch)) as pbar_epoch:
        for epoch in pbar_epoch:
            train_loss = trainer.process_epoch(train_loader, epoch=epoch)
            
            with torch.no_grad():
                test_loss = trainer.process_epoch(test_loader, epoch=epoch, training=False)
