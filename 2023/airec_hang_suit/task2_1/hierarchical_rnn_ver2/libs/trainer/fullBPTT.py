...
class fullBPTTtrainer:
    ...
    def __init__(self, 
                 ...
                 optimizer, 
                 # loss_weights = [0.1, 1]
                 loss_w_dic:Dict[str, float]={"i": 0.1, "k": 0.1, "v": 1.0}, 
                 device="cpu"):
        self.device = device
    ...
    def process_epoch(self, data, training=True):
        ...
        total_loss = 0.0
        for n_batch, ((x_img, x_vec), (y_img, y_vec)) in enumerate(data):   # x_joint, y_joint
            ...
            states = None
            yi_hat_list, yv_hat_list = [], []
            dec_pts_list, enc_pts_list = [], []
            T = x_img.shape[1]
            for t in range(T - 1):
                yi_hat, yv_hat, enc_pts, dec_pts, states = self.model(
                    x_img[:, t], x_vec[:, t], states
                )
            ...
            enc_pts_data = torch.stack(dec_pts_list[:-1])
            dec_pts_data = torch.stack(enc_pts_list[1:])
            ...
            pt_loss = criterion(enc_pts_data, dec_pts_data)  * self.loss_w_dic["k"] # * self.scheduler(self.loss_weights[2])
