...
class HSARNN(nn.Module):
    def __init__(
        ...
        # self.rec
        self.hrnn = HierarchicalRNNCell(srnn_input_dims={"k": key_dim, "v":vec_dim}, 
    ...
    def forward(self, xi, xv, states=None):
        # Encode input image
        # im_hid
        ...
        states = self.hrnn(xk, xv, states) # enc_pts
        # y_joint = self.decoder_joint(rnn_hid[0][0])  # Decode joint prediction
        # dec_pts = self.decoder_point(rnn_hid[1][0])  # Decode points
        yv = self.decoder_joint(states[0][0])  # Decode joint prediction
        dec_pts = self.decoder_point(states[1][0])  # Decode points
        ...
        return yi, yv, enc_pts, dec_pts, states# rnn_hid
