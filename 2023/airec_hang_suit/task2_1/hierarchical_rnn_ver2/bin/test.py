...
# test data
# img
raw_img_data = np.load(f"{data_dir_path}/test/left_img.npy")
...
img_data = normalization(_img_data.astype(np.float32), (0.0, 255.0), minmax)
...
xv_data = yv_data = torch.from_numpy(vec_data).float()
...
for t in range(T-1):
    ...
    yi_hat, yv_hat, enc_pts, dec_pts, states = model(
        xi_data[:, t], xv_data[:, t], states
    )
...
vec_loss = (yv_hat_data - yv_data[:, 1:])**2 * loss_w_dic["v"]
