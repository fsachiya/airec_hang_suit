...
class SARNN(nn.Module):
    ...
    def __init__(
        ...
        input_dim = self.k_dim * 2  # pos has x, y
        self.lstm = nn.LSTMCell(input_dim, hid_dim)  # LSTM cell

        # Point Decoder
        self.decoder_point = nn.Sequential(
            nn.Linear(hid_dim, self.k_dim * 2), 
            activation
    ...
    def forward(self, xi, state=None):
        """
        ...
        new_state = self.lstm(hid, state)  # LSTM forward pass
        dec_pts = self.decoder_point(new_state[0])  # Decode points

        # Reshape decoded points
        dec_pts_in = dec_pts.reshape(-1, self.k_dim, 2)
        heatmap = self.issm(dec_pts_in)  # Inverse Spatial Softmax
        hid = torch.mul(heatmap, img_feat)  # Multiply heatmap with image feature `im_hid`

        yi = self.decoder_image(hid)  # Decode image
        return yi, enc_pts, dec_pts, new_state
