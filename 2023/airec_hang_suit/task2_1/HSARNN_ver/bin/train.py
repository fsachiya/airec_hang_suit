...
dir_name = "/home/fujita/job/2023/airec_hang_suit/rosbag/HangSuit_task2_1"
bounds = np.load(f'{dir_name}/param/arm_joint_bounds.npy')

arm_angles = np.load(f'{dir_name}/train/joint_state.npy')
norm_arm_angles = normalization(arm_angles, bounds, minmax)[:, :, 7:]
right_hand = np.load(f'{dir_name}/train/hand_cmd.npy')
right_hand = np.apply_along_axis(cos_interpolation, 1, right_hand, step=10)
norm_right_hand = normalization(right_hand, (0.0, 1.0), minmax)
# right_hand = smoothing(np.load(f'{dir_name}/train/hand_cmd.npy')[:, :, 4:])
joints = np.concatenate((norm_arm_angles, norm_right_hand), axis=-1)
_img = np.load(f'{dir_name}/train/left_img.npy')
...
# test data
arm_angles = np.load(f'{dir_name}/test/joint_state.npy')
norm_arm_angles = normalization(arm_angles, bounds, minmax)[:, :, 7:]
right_hand = np.load(f'{dir_name}/test/hand_cmd.npy')
right_hand = np.apply_along_axis(cos_interpolation, 1, right_hand, step=10)
norm_right_hand = normalization(right_hand, (0.0, 1.0), minmax)
# right_hand = smoothing(np.load(f'{dir_name}/test/hand_cmd.npy')[:, :, 4:])
joints = np.concatenate((norm_arm_angles, norm_right_hand), axis=-1)
_img = np.load(f'{dir_name}/test/left_img.npy')
...
images = np.transpose(images, (0, 1, 4, 2, 3))
test_dataset = MultimodalDataset(images, joints, device=device, stdev=stdev)
