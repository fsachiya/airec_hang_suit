...
try:
    from libs.layer import PSMTRNNVTBCell
except:
    sys.path.append("./libs/")
    from layer import PSMTRNNVTBCell


class CNNPSMTRNNVTB(nn.Module):
    def __init__(
        self, 
        context_size:dict[str, int], 
        left_fast_tau_range:dict[str, int],
        right_fast_tau_range:dict[str, int],
        slow_tau:torch.Tensor, 
        feat_size=10,
        trainInitialValue=True
    ):
        super(CNNPSMTRNNVTB, self).__init__()
        ...
        # RNN
        input_size   = 10+7+3 # left or right
        out_img_size = 100
        out_vec_size = 7
        out_state_size = 3

        self.h2h = PSMTRNNVTBCell(  input_size, 
                                    self.context_size,
                                    left_fast_tau_range,
    ...
    def forward(self, 
        ...
        # Encoder
        lxi_feat = self.encoder(xi["left"])
        rxi_feat = self.encoder(xi["right"])

        # RNN
        # x = torch.cat((xi_feat, xv, xs), dim=1)
        lx = torch.cat((lxi_feat, xv["left"], xs["left"]), dim=1)    # , xdv["left"]
        rx = torch.cat((rxi_feat, xv["right"], xs["right"]), dim=1)  # , xdv["right"]
        x = {"left": lx, "right": rx}
        
        state, fast_tau = self.h2h(x, state)
        ...
        return yi, yv, ys, state, fast_tau
