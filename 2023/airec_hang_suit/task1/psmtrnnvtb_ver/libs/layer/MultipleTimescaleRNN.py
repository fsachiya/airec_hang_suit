...
class MTRNNCell(nn.Module):
    ...
    def __init__(
        ...
        # Input Layers
        self.i2f = nn.Linear(self.input_dim, self.fast_dim, bias=use_bias)

        # Fast context layer
        self.f2f = nn.Linear(self.fast_dim, self.fast_dim, bias=False)
        self.f2s = nn.Linear(self.fast_dim, self.slow_dim, bias=use_bias)

        # Slow context layer
        self.s2s = nn.Linear(self.slow_dim, self.slow_dim, bias=False)
        self.s2f = nn.Linear(self.slow_dim, self.fast_dim, bias=use_bias)
