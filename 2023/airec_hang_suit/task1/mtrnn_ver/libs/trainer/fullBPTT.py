...
class fullBPTTtrainer:
    ...
    def process_epoch(self, data, training=True):
        ...
        total_loss, total_img_loss, total_joint_loss, total_state_loss = 0.0, 0.0, 0.0, 0.0
        for n_batch, ((x_img, x_joint, x_state), 
                      (y_img, y_joint, y_state)) in enumerate(data):
            xi_data = x_img.to(self.device)
            yi_data = y_img.to(self.device)
            
            xv_data = x_joint.to(self.device)
            yv_data = y_joint.to(self.device)
            xs_data = x_state.to(self.device)
            ys_data = y_state.to(self.device)
            
            state = None
            yi_hat_list, yv_hat_list, ys_hat_list = [], [], []
            T = xi_data.shape[1]
            for t in range(T - 1):
                yi_hat, yv_hat, ys_hat, state = self.model(
            ...
            joint_loss = nn.MSELoss()(yv_hat_data, yv_data[:, 1:]) * self.loss_weights[1]
            state_loss = nn.MSELoss()(ys_hat_data, ys_data[:, 1:]) * self.loss_weights[2]
        ...
        total_loss_dir = {"total_loss": total_loss/(n_batch + 1),
                          ...
                          "total_state_loss": total_state_loss/(n_batch + 1),
                          }
